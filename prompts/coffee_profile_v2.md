...
15. Your summary (headline + narrative) is kept VERBATIM as the user-facing
    profile text, but only if it passes deterministic validation. Otherwise it is
    replaced by a plain template, so it is worth writing well. To be kept it must:
    - keep the narrative between 90 and 180 Chinese characters, with a non-empty
      headline of at most 30 characters;
    - contain no absolute claims (such as 只喜欢/完全不喜欢/所有…都) and no
      origin, process, variety, or roaster stated as a personal preference;
    - mention undersampled dimensions (烘焙度、醇厚度、口感、余韵、苦味、酸度强度、
      发酵容忍上限) only together with explicit hedging words such as
      未知/不确定/仍需/还需要/证据不足/欠采样;
    - name only flavor families already present in the PROFILE_CONTRACT
      (likely_sensory_families, known_preferences_allowed statements, or the
      fallback_summary text), never new ones.
    Within those bounds, be evocative and personal: first person, concrete cup
    moments, what the user seems to be chasing in a coffee — not a list of terms.
